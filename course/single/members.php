...
if(count($students_undertaking) > 0 ){
	?>
	<h4 class="total_students"><?php _e("Nombre total d'étudiants dans ce cours",'vibe'); ?><span><?php echo vibe_sanitizer($students,'text'); ?></span></h4>
	<form id="course_user_ajax_search_results" data-id="<?php echo get_the_ID(); ?>">
		<select id="course_status">
			<option value=""><?php _ex('Filtrer par Statut','course-members-select','vibe'); ?></option>
			<?php
				$sorters = apply_filters('wplms_course_members_sorters',array(
					'recent'=> _x('A récemment rejoint','course-members-select','vibe'),
					'alphabetical'=> _x('Alphabétique','course-members-select','vibe'),
		...
		<span id="search_course_member">
			<input type="text" name="search" placeholder="<?php _e("Entrez le nom/email de l'étudiant",'vibe'); ?>">		</span>
	 </form>
	<?php
	...
	echo '<div class="message">'._x('Aucun membre trouvé dans ce cours.','No members notification in course - members','vibe').'</div>';
}
