...
<div class="item-list-tabs activity-type-tabs no-ajax" id="subnav" role="navigation">
	<ul>
		...
		if($flag){ ?>
		<li id="activity-course_<?php echo vibe_sanitizer($bp->current_item,'text'); ?>" class="selected"><a href="#" title="all-activity"><?php _e('Toutes les activités','vibe'); ?></a></li>
		<?php } ?>
		<?php
		if(is_user_logged_in() && bp_course_is_member()){ ?>
		<li id="activity-course_personal_<?php echo vibe_sanitizer($bp->current_item,'text'); ?>"><a href="#" title="<?php _e('My Activity','vibe'); ?>"><?php _e('Mon Activité','vibe'); ?></a></li>
		<?php } ?>
		<li id="activity-filter-select" class="last">
			<label for="activity-filter-by"><?php _e( 'Montrez:', 'vibe' ); ?></label>
			<select id="activity-filter-by">
				<option value="-1"><?php _e( '&mdash; Tous &mdash;', 'vibe' ); ?></option>
...
<?php if ( bp_has_activities( bp_ajax_querystring( 'activity' ) ) ) : ?>
	...
	<?php if ( bp_activity_has_more_items() ) : ?>
		...
		<li class="load-more">
			<a href="#more"><?php _e( 'Chargez Plus', 'vibe' ); ?></a>
		</li>
...
<?php else : ?>
	...
	<div id="message" class="info">
		<p><?php _e( "Désolé, Aucune activité n'a été trouvée. Veuillez essayer un autre filtre.", 'vibe' ); ?></p>
	</div>
